...
/// Expanded key size: 4 * (14 + 1) = 60 u32 words for AES-256.
const EXPANDED_KEY_SIZE: usize = 60;

/// Number of rounds for AES-256.
const NR: usize = 14;

/// Number of 32-bit words in the key for AES-256.
const NK: usize = 8;
...
impl ExpandedKey {
    ...
    /// Create a decryption expanded key from the raw 32-byte key.
    #[inline]
...
mod aesni {
    ...
    /// Check at runtime whether AES-NI is available.
    #[inline]
    ...
    unsafe fn load_round_key_from_words(
        ...
        for i in 0..4 {
            let be = expanded_key[base + i].to_be_bytes();
            bytes[i * 4] = be[0];
            bytes[i * 4 + 1] = be[1];
            bytes[i * 4 + 2] = be[2];
            bytes[i * 4 + 3] = be[3];
    ...
    /// Encrypt 4 blocks (64 bytes) simultaneously using AES-NI interleaving.
    ///
    /// SAFETY: caller must ensure AES-NI is available.
    #[target_feature(enable = "aes", enable = "sse2")]
    pub unsafe fn encrypt_block_x4(
        ...
        _mm_storeu_si128(output[0..16].as_mut_ptr() as *mut __m128i, s0);
        _mm_storeu_si128(output[16..32].as_mut_ptr() as *mut __m128i, s1);
        _mm_storeu_si128(output[32..48].as_mut_ptr() as *mut __m128i, s2);
        _mm_storeu_si128(output[48..64].as_mut_ptr() as *mut __m128i, s3);
    }

    /// Decrypt a single 16-byte block using AES-NI.
    ///
    /// SAFETY: caller must ensure AES-NI is available.
    #[target_feature(enable = "aes", enable = "sse2")]
    ...
    /// Decrypt 4 blocks (64 bytes) simultaneously using AES-NI interleaving.
    #[target_feature(enable = "aes", enable = "sse2")]
    ...
    /// SAFETY: caller must ensure AES-NI is available.
    #[target_feature(enable = "aes", enable = "sse2")]
    pub unsafe fn prepare_dec_key(expanded_key: &mut [u32; EXPANDED_KEY_SIZE]) {
        let num_rounds = NR;
        for i in 0..num_rounds / 2 {
            let j = num_rounds - i;
            for k in 0..4 {
                expanded_key.swap(i * 4 + k, j * 4 + k);
            }
        }
        ...
        for i in 1..num_rounds {
            ...
            for j in 0..4 {
                expanded_key[i * 4 + j] = u32::from_be_bytes([
                    bytes[j * 4],
                    bytes[j * 4 + 1],
                    bytes[j * 4 + 2],
                    bytes[j * 4 + 3],
...
#[inline(always)]
fn put_u32_be(buf: &mut [u8], val: u32) {
...
pub fn set_encryption_key(key: &[u8; 32], expanded_key: &mut [u32; EXPANDED_KEY_SIZE]) {
    for i in 0..NK {
        expanded_key[i] = get_u32_be(&key[4 * i..]);
    }
...
/// Expand a 32-byte key into a decryption key schedule.
pub fn set_decryption_key(key: &[u8; 32], expanded_key: &mut [u32; EXPANDED_KEY_SIZE]) {
    set_encryption_key(key, expanded_key);
    ...
    for i in (4..NR * 4).step_by(1) {
        ...
        expanded_key[i] = TD0[TE1[(ek >> 24) as usize] as usize & 0xff]
            ...
            ^ TD2[TE1[((ek >> 8) & 0xff) as usize] as usize & 0xff]
            ^ TD3[TE1[(ek & 0xff) as usize] as usize & 0xff];
    }
}
...
pub fn encrypt_block_soft(input: &[u8; 16], output: &mut [u8; 16], key: &[u32; EXPANDED_KEY_SIZE]) {
    let mut s0 = get_u32_be(&input[0..]) ^ key[0];
    let mut s1 = get_u32_be(&input[4..]) ^ key[1];
    let mut s2 = get_u32_be(&input[8..]) ^ key[2];
    ...
    s0 = ((TE2[(t0 >> 24) as usize] & 0xff000000)
        ^ (TE3[((t1 >> 16) & 0xff) as usize] & 0x00ff0000)
        ^ (TE0[((t2 >> 8) & 0xff) as usize] & 0x0000ff00)
        ^ (TE1[(t3 & 0xff) as usize] & 0x000000ff))
        ^ key[ki];
    put_u32_be(&mut output[0..4], s0);

    s1 = ((TE2[(t1 >> 24) as usize] & 0xff000000)
        ^ (TE3[((t2 >> 16) & 0xff) as usize] & 0x00ff0000)
        ^ (TE0[((t3 >> 8) & 0xff) as usize] & 0x0000ff00)
        ^ (TE1[(t0 & 0xff) as usize] & 0x000000ff))
        ^ key[ki + 1];
    put_u32_be(&mut output[4..8], s1);

    s2 = ((TE2[(t2 >> 24) as usize] & 0xff000000)
        ^ (TE3[((t3 >> 16) & 0xff) as usize] & 0x00ff0000)
        ^ (TE0[((t0 >> 8) & 0xff) as usize] & 0x0000ff00)
        ^ (TE1[(t1 & 0xff) as usize] & 0x000000ff))
        ^ key[ki + 2];
    put_u32_be(&mut output[8..12], s2);

    s3 = ((TE2[(t3 >> 24) as usize] & 0xff000000)
        ^ (TE3[((t0 >> 16) & 0xff) as usize] & 0x00ff0000)
        ^ (TE0[((t1 >> 8) & 0xff) as usize] & 0x0000ff00)
        ^ (TE1[(t2 & 0xff) as usize] & 0x000000ff))
        ^ key[ki + 3];
    put_u32_be(&mut output[12..16], s3);
}

/// Decrypt a single 16-byte block using the software T-table implementation.
pub fn decrypt_block_soft(input: &[u8; 16], output: &mut [u8; 16], key: &[u32; EXPANDED_KEY_SIZE]) {
    let mut s0 = get_u32_be(&input[0..]) ^ key[0];
    let mut s1 = get_u32_be(&input[4..]) ^ key[1];
    let mut s2 = get_u32_be(&input[8..]) ^ key[2];
    let mut s3 = get_u32_be(&input[12..]) ^ key[3];
    ...
    s0 = ((SBOX_INV[(t0 >> 24) as usize] as u32) << 24)
        ^ ((SBOX_INV[((t3 >> 16) & 0xff) as usize] as u32) << 16)
        ^ ((SBOX_INV[((t2 >> 8) & 0xff) as usize] as u32) << 8)
        ^ (SBOX_INV[(t1 & 0xff) as usize] as u32)
        ^ key[ki];
    put_u32_be(&mut output[0..4], s0);

    s1 = ((SBOX_INV[(t1 >> 24) as usize] as u32) << 24)
        ^ ((SBOX_INV[((t0 >> 16) & 0xff) as usize] as u32) << 16)
        ^ ((SBOX_INV[((t3 >> 8) & 0xff) as usize] as u32) << 8)
        ^ (SBOX_INV[(t2 & 0xff) as usize] as u32)
        ^ key[ki + 1];
    put_u32_be(&mut output[4..8], s1);

    s2 = ((SBOX_INV[(t2 >> 24) as usize] as u32) << 24)
        ^ ((SBOX_INV[((t1 >> 16) & 0xff) as usize] as u32) << 16)
        ^ ((SBOX_INV[((t0 >> 8) & 0xff) as usize] as u32) << 8)
        ^ (SBOX_INV[(t3 & 0xff) as usize] as u32)
        ^ key[ki + 2];
    put_u32_be(&mut output[8..12], s2);

    s3 = ((SBOX_INV[(t3 >> 24) as usize] as u32) << 24)
        ^ ((SBOX_INV[((t2 >> 16) & 0xff) as usize] as u32) << 16)
        ^ ((SBOX_INV[((t1 >> 8) & 0xff) as usize] as u32) << 8)
        ^ (SBOX_INV[(t0 & 0xff) as usize] as u32)
        ^ key[ki + 3];
    put_u32_be(&mut output[12..16], s3);
}
